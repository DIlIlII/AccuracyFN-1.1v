...
namespace rend {
	...
	VOID EndScene(ImGuiWindow& window) 
	{
		...
		if (showMenu)
		{
			if (ImGui::Begin(xorstr("AccuracyFN 1.1v (LEGIT) & (RAGE)     [Leaked Version]"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize))
			{
				ImGui::Separator();

				if (ImGui::Button("Join Renox Community Discord <3"))
				{
					system("start https://discord.gg/JXUnvD2f7s");
				}

				ImGui::Separator();

				ImGui::Text("BTW Renox Community Is The Best Server!");

				ImGui::Text("Join There Server!");

				ImGui::Separator();

				ImGui::Text("Join: https://discord.gg/JXUnvD2f7s");
				...
				ImGui::SliderInt(xorstr("Fov Slider##slider"), &Settings.FovValue, 80.0f, 200.0f, xorstr("%.2f"));
				...
				if (ImGui::CollapsingHeader("Player ESP"))
				{
					...
					ImGui::Checkbox("OneLine ESP (LessLag)", &Settings.OneLine);

					ImGui::Checkbox("Player ESP TEXT (NoLag)", &Settings.PlayerEspText);
					...
					ImGui::Checkbox("Trading BOT ESP", &Settings.TradingBotEsp);
				}
				...
				if (ImGui::CollapsingHeader("Crosshair Options"))
				{
					...
					ImGui::Checkbox("Crosshair (Red)", &Settings.CrossRed);

					ImGui::Checkbox("Crosshair (Green)", &Settings.CrossGreen);

					ImGui::Checkbox("Crosshair (Blue)", &Settings.CrossBlue);

					ImGui::Checkbox("Crosshair (Purple)", &Settings.CrossPurple);
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		if (Settings.WaterMark)
		{
			...
			DrawOutlinedText(m_pFont, TextFormat(xorstr("Cracked By Maximus")), ImVec2(Settings.watermarkx, Settings.watermarky), 25.f, WaterMarkColor, true);
		}
...
*/
				for (auto li = 0UL; li < ReadDWORD(world, offsets::World::Levels + sizeof(PVOID)); ++li) {
					...
					for (auto ai = 0UL; ai < ReadDWORD(level, offsets::Level::AActors + sizeof(PVOID)); ++ai) {
						...
						else if (Settings.fishingholeesp && wcsstr(name.c_str(), xorstr(L"BGA_Athena_FlopperSpawn_C"))) // so pasted
					 {
	...
	BOOLEAN Initialize()
	{
		...
		sd.OutputWindow = FindWindowW(xorstr(L"UnrealWindow"), xorstr(L"Fortnite  "));
		...
		if (FAILED(D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, 0, 0, &featureLevel, 1, D3D11_SDK_VERSION, &sd, &swapChain, &device, nullptr, &context))) {
			MessageBox(0, xorstr(L"failed: dx11"), xorstr(L"Psdasfr43"), MB_ICONERROR);
			return FALSE;
		}
